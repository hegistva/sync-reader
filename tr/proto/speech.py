...
def alignAudio(lang, audio_file, transcript_file):
    ...
    if os.path.exists(audio_file) and os.path.exists(transcript_file):
        ...
        ret = subprocess.call(['java', '-jar', ALIGNER, lang, audio_file, transcript_file, outfile], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
        if ret == 0: # success
...
chapter = 1
outfile = os.path.join(TEMP_DIR, 'chapter_%04d.audio.map' % chapter)
alignChapter(utils.Lang.FRA, book_id, chapter, outfile)
